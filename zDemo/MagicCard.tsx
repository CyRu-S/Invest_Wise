...
interface MagicCardProps {
    ...
    enableStars?: boolean;
    textAutoHide?: boolean; // added compatibility prop, though not strictly needed if children are custom
}
...
export const MagicCard = ({
    ...
    }, [animateParticles, clearAllParticles, disableAnimations, isMobile, enableTilt, enableMagnetism, clickEffect, glowColor]);
    ...
    // Add the card--border-glow class logic here equivalent to the CSS provided
    return (
        <div
            ref={cardRef}
            className={`relative overflow-hidden card--border-glow ${className}`}
            style={baseCardStyle}
        >
            {/* Particle effect container helper (pseudo elements are handled by CSS class but we add explicit styles if needed) */}
            <style>
                {`
...
const GlobalSpotlight = ({
    ...
    useEffect(() => {
        ...
        // cleanup previous spotlight if any exists to avoid duplicates on re-renders
    ...
    `;
        ...
        const handleMouseMove = (e: MouseEvent) => {
            ...
            // Use standard class selector
            const cards = section.querySelectorAll('.card--border-glow');
        ...
        const handleMouseLeave = () => {
            ...
            if (gridRef.current) {
                gridRef.current.querySelectorAll('.card--border-glow').forEach(card => {
                    (card as HTMLElement).style.setProperty('--glow-intensity', '0');
                });
            }
