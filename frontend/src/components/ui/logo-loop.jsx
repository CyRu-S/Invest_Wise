...
function LogoChip({ logo }) {
  return (
    <div className="group flex shrink-0 items-center gap-3 rounded-full border border-white/10 bg-white/5 px-5 py-3 text-white/85 shadow-[0_14px_40px_rgba(4,8,24,0.22)] backdrop-blur-xl transition-all duration-300 hover:border-indigo-400/30 hover:bg-white/8 hover:-translate-y-0.5">
      <span
        className={`flex h-11 w-11 items-center justify-center rounded-full bg-gradient-to-br ${logo.gradient} text-sm font-bold tracking-[0.18em] text-white shadow-[inset_0_1px_0_rgba(255,255,255,0.22)]`}
      >
        {logo.mark}
      </span>
      <div className="min-w-0">
        <p className="text-[0.8rem] font-semibold uppercase tracking-[0.22em] text-white/45">{logo.tag}</p>
        <p className="text-base font-semibold tracking-tight text-white/88">{logo.name}</p>
      </div>
    </div>
  );
}
